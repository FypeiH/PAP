...
if($txtpesquisar == '')
{
  $res = $pdo->query("SELECT * from utilizadores where nivel = 'Médico' order by id desc LIMIT $limite, $itens_por_pagina");
}
else
{
  $txtpesquisar = '%'.@$_POST['txtpesquisar'].'%';
  $res = $pdo->query("SELECT * from utilizadores where nivel = 'Médico' and nome LIKE '$txtpesquisar' or cedula LIKE '$txtpesquisar' order by id desc");
}
...
$res_todos = $pdo->query("SELECT * from utilizadores where nivel = 'Médico'");
...
for($i=0; $i < count($dados); $i++)
{
  ...
  $resultado_esp = $pdo->query("SELECT * from especialidades where id = '$especialidade'");
  $dados_esp = $resultado_esp->fetchAll(PDO::FETCH_ASSOC);
