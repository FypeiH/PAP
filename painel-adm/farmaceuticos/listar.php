...
if($txtpesquisar == ''){
  $res = $pdo->query("SELECT * from utilizadores where nivel = 'Farmacêutico' order by id desc LIMIT $limite, $itens_por_pagina");
}else{
  $txtpesquisar = '%'.@$_POST['txtpesquisar'].'%';
  $res = $pdo->query("SELECT * from utilizadores where nivel = 'Farmacêutico' and nome LIKE '$txtpesquisar'  order by id desc");
...
$res_todos = $pdo->query("SELECT * from utilizadores where nivel = 'Farmacêutico'");
