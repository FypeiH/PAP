...
//Buscar a página atual
  ...
  if($txtpesquisar == ''){
    $res = $pdo->query("SELECT * from utilizadores where nivel = 'Admin' order by id desc LIMIT $limite, $itens_por_pagina");
  }else{
    $txtpesquisar = '%'.@$_POST['txtpesquisar'].'%';
    $res = $pdo->query("SELECT * from utilizadores where nivel = 'Admin' and nome LIKE '$txtpesquisar'  order by id desc");
  ...
  //Totalizar os Registos para a Paginação 
    $res_todos = $pdo->query("SELECT * from utilizadores where nivel = 'Admin'");
