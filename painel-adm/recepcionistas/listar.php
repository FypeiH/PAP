...
if($txtpesquisar == ''){
  $res = $pdo->query("SELECT * from utilizadores where nivel = 'Recepcionista' order by id desc LIMIT $limite, $itens_por_pagina");
}else{
  $txtpesquisar = '%'.@$_POST['txtpesquisar'].'%';
  $res = $pdo->query("SELECT * from utilizadores where nivel = 'Recepcionista' and nome LIKE '$txtpesquisar'  order by id desc");
...
$res_todos = $pdo->query("SELECT * from utilizadores where nivel = 'Recepcionista'");
